...
#define X_mWriteReg(BASE_ADDRESS, RegOffset, data)  \
  *(volatile unsigned int *)(BASE_ADDRESS + RegOffset) = ((unsigned int) data);
#define X_mReadReg(BASE_ADDRESS, RegOffset) \
  *(volatile unsigned int *)(BASE_ADDRESS + RegOffset);
#define XUartChanged_IsTransmitFull(BASE_ADDRESS)  \
  ((*(volatile unsigned int *)((unsigned int)BASE_ADDRESS + 0x2C) & 0x10) == 0x10)
